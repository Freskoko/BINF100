...
def translate(sequence: str) -> str:
    aa_seq = ""
    for i in range(0, len(sequence)-3, 3): #fra 0 til len(seq)-3, hopp over 3 hver gang
...
def compare_sequences(outfile: TextIOWrapper, seq1: str, seq2: str):
    ...
    for index,aas in enumerate(zip(seq1,seq2)):
        ...
        if aas[0] != aas[1]:
            ...
            outstr = f"Mismatch found at index {index}: seq1: {aas[0]}, seq2: {aas[1]} \n"
            print(outstr)
...
def main(fname):
    with open("src/inputs/wild_type.txt", "r") as person1, open(f"src/inputs/{fname}.txt", "r") as person2, open(f"src/outputs/outfile_{fname}.txt", "w") as outfile:
